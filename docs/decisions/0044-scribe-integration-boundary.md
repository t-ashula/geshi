...
- 現行 `scribe` は `/home/office/src/github.com/t-ashula/scribe` にある Python / FastAPI service で，`POST /transcribe` に `audio/x-wav` を upload して非同期 job を起票し，`GET /transcribe/{request_id}` を polling して結果を得る API 形を持つ
...
- `tmp/v0.3.0/packages/scribe-client` に TypeScript client 実装の残骸はあるが，既定 base URL が `http://localhost:8002` で `scribe` README の起動例 `8000` とずれており，`pending` / `working` / `error` を first-class な戻り値として持たないなど，そのまま再利用するには前提差分がある
...
- 既存の `tmp/v0.3.0/packages/scribe-client` は参考実装として扱ってよいが，現行 API との差分を吸収せずに直接 product code へ持ち込まない
...
- polling 型 job API を `geshi` 側の内部 job / state 管理へどう写像するかを先に固定しやすくなる
